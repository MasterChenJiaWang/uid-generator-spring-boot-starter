 ...
 */
package com.daren.chen.uid.buffer;
...
import com.daren.chen.uid.utils.PaddedAtomicLong;

/**
 * Represents a ring buffer based on array.<br>
 * Using array could improve read element performance due to the CUP cache line. To prevent the side effect of False
 * Sharing, {@link PaddedAtomicLong} is using on 'tail' and 'cursor'
 * <p>
 * <p>
...
public class RingBuffer {
     ...
     *            must be positive & a power of 2
     * @param paddingFactor
     *            percent in (0 - 100). When the count of rest available UIDs reach the threshold, it will trigger
     *            padding buffer<br>
     ...
     * and put the one by one into the buffer, so it is unnecessary put in multi-threads
     *
     * @param uid
     ...
     * <p>
     * <p>
     * Before getting the UID, we also check whether reach the padding threshold, the padding buffer operation will be
     * triggered in another thread<br>
